...
1. **Admin/Owner logs in and reaches `Main page`**
   - From `Authorization page` → successful authorization → `Pop-up “2FA”` (when applicable).
   - From `Pop-up “2FA”` → `successfully passed 2FA` → `Main page`.
2. **Open `Roles` section**
   ...
   - On entry, user sees a **list of users** (members) with fields:
     - Id, Name & Surname, Role (or roles), Gmail (hidden by default, can be shown), Password (hidden by default; can be visible only when auto-generated via `Show`), Notes, Timestamp when created (approved creation).
...
- **Authorization failures:**
  - `authorization fail (wrong password/email)` loops back to the `Authorization page`.
- **2FA failures:**
  - `2FA fail (wrong code for the 2FA)` loops back to the `Authorization page`.
- **Captcha failures:**
...
- **Ambiguity:** Some role boxes (e.g. `If role is “Media”`, `If role is “External”`) appear near the `Roles` area but are not directly connected to it in the visible excerpt.
  - **Why unclear:** The diagram shows these boxes nearby, but the connector wiring into `Roles` is not present/fully visible, so the actual access effect is uncertain.
  - **Most likely reading:** These roles are handled elsewhere or have no access to the `Roles` hub in this fragment.
  - **Alternative reading:** Connectors to `Roles` exist outside the captured/visible portion of the board.
- **Ambiguity:** The board fragment describes additional roles being defined near conditions, but it only explicitly shows `Roles` access for `admin/owner/moderator`.
  - **Why unclear:** The fragment states that other roles are “defined near role conditions” but does not show them as having a connector to `Roles`.
  - **Most likely reading:** Only the shown roles have access (others may be intentionally restricted).
  - **Alternative reading:** Other roles may also connect to `Roles`, but those connectors are missing from the visible excerpt.
