...
A comprehensive guide for creating animations that feel right, distilled from Emil Kowalski's "Animations on the Web" course and personal site.

> Practical step-by-step implementations live in `references/web-animations/` — load the relevant file from there when you need code patterns for a specific scenario.

---

## Initial Response

When this reference is first invoked without a specific question, respond only with:
...
When reviewing animations, always use a markdown table. Never use a "Before:" / "After:" list.
...
Every animation decision starts with these four questions:
...
See `references/web-animations/` for detailed implementations of each scenario below.

| Scenario | Solution | File |
|---|---|---|
| Make buttons feel responsive | `transform: scale(0.97)` on `:active` | `practical-tips.md` |
| Element appears from nowhere | Start from `scale(0.95)`, not `scale(0)` | `practical-tips.md` |
| Shaky/jittery animations | Add `will-change: transform` | `practical-tips.md` |
| Hover causes flicker | Animate child element, not parent | `practical-tips.md` |
| Popover scales from wrong point | Set `transform-origin` to trigger location | `practical-tips.md` |
| Sequential tooltips feel slow | Skip delay/animation after first tooltip | `practical-tips.md` |
| Small buttons hard to tap | Use 44px minimum hit area (pseudo-element) | `practical-tips.md` |
| Something still feels off | Add subtle blur (under 20px) to mask it | `practical-tips.md` |
| Hover triggers on mobile | Use `@media (hover: hover) and (pointer: fine)` | `practical-tips.md` |
